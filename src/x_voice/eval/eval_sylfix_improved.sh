...
ckpt=600000 # ckpt=15500

# Stage 1 uses reference transcript
drop_text=False

test_set="zh id ja ko th vi en fr de it pt ru es bg hr cs da nl et fi el hu lv lt mt pl ro sk sl sv"
ref_set="zh id ja ko th vi en fr de it pt ru es bg hr cs da nl et fi el hu lv lt mt pl ro sk sl sv"
...
if [ "${decoupled}" = "True" ]; then
    ...
    decode_dir="${PROJECT_ROOT}/results/${exp_name}_${ckpt}_ko_frontend/${dataset}/${sp_type}_seed${seed}_euler_nfe${nfe}_vocos_ss-1_schedule${cfg_schedule}_cfgI${cfg_strength}_cfgII${cfg_strength2}_speed${speed}zero_shot"

else

    decode_dir="${PROJECT_ROOT}/results/${exp_name}_${ckpt}_ko_frontend/${dataset}/${sp_type}_seed${seed}_euler_nfe${nfe}_vocos_ss-1_schedule${cfg_schedule}_cfg${cfg_strength}_speed${speed}zero_shot"
...
accelerate launch \
    ...
    "${PROJECT_ROOT}/src/x_voice/eval/eval_infer_batch_improved.py" \
    -s ${seed} \
    -n "${exp_name}" \
    -c ${ckpt} \
    -t "${dataset}" \
