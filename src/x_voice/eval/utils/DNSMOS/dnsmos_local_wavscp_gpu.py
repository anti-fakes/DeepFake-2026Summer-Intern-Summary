...
def main(args):
    ...
    with open(args.testset_scp, 'r') as f:
        for line in f:
            sc = '\t' if '\t' in line else ' '
            wid, path = line.strip().split(sc, 1)
            clips.append(path)

    p808_model_path = os.path.join(f'{env_root}/DNSMOS', 'model_v8.onnx')

    if args.personalized_MOS:
        primary_model_path = os.path.join(f'{env_root}/pDNSMOS', 'sig_bak_ovr.onnx')
    else:
        primary_model_path = os.path.join(f'{env_root}/DNSMOS', 'sig_bak_ovr.onnx')
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in pbar:
            ...
            try:
                ...
                pbar.write(f"[ERROR] {clip} generated an exception: {exc}")
            else:
                rows.append(data)
...
if __name__=="__main__":
    ...
    parser.add_argument('-t', "--testset_scp", default='.',
    ...
    parser.add_argument('-e', "--env_root", default='.',
                        help='Flag to indicate if personalized MOS score is needed or regular')
