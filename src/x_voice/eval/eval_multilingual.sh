...
# ===== Stage1 baseline =====
# exp_name=XVoice_Base_Stage1
# ckpt=600000
exp_name=XVoice_KO_Replay_SylFix_Stage1
ckpt=15500

# Stage1: reference text 사용
drop_text=False

# Korean intra-lingual evaluation
test_set="ko en"
ref_set="en ko"
...
# Stage1 duration estimation
sp_type=utf # syllable, pretrained

# sp_type=utf에서는 SRP를 사용하지 않지만
# 원본 script argument 구조는 그대로 유지
...
for ((i=0; i<${#test_langs[@]}; i++)); do
    ...
    #
    # cat ${decode_dir}/${ref_lang}_${lang}/mos.csv \
    #     | sed '1d' \
    #     | awk -F ',' \
    #       '{ sum += $NF; count++ } END { if (count > 0) print sum / count }' \
    #     > ${decode_dir}/${ref_lang}_${lang}/dnsmos_mean.txt
